...
from vocab import import_nouns, import_adjectives, import_verbs, filter_vocab_by_repos
...
adjective_vocab = filter_vocab_by_repos(import_adjectives(), ("alap1", "alap2"))
verb_vocab = filter_vocab_by_repos(import_verbs(), ("alap1", "alap2"))
...
def check_recognition_answer(answer_key):
    ...
    if correct:
        ...
        st.session_state.answer_display_message = feedback_box(
            f'<strong>Helytelen válasz. A helyes válasz: <span style="font-weight:900;">{correct_answer_label}</span>.</strong>',
            "incorrect",
        )
    record = {
