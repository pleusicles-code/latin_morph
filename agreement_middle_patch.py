...
'''
    ...
    # Number switching is integrated into the same transformed recognition block.
    source = source.replace(
    ...
    ns_display_anchor = '''            if multiple_answer_message:
                supplementary.append(multiple_answer_message)
'''
    ns_display = '''            if multiple_answer_message:
                supplementary.append(multiple_answer_message)
                ...
                for source_number, source_case in sorted(matching_analyses):
                    ...
                    for noun_target in _ar_forms(build_noun([noun, source_case, target_number])):
                        for adjective_target in _ar_forms(
                            ...
                            for target_phrase in (f"{n} {a}", f"{a} {n}"):
                                key = target_phrase.casefold()
                                if key not in seen_targets:
                                    seen_targets.add(key)
                                    target_phrases.append(target_phrase)
                st.session_state.correct_answer = target_phrases
'''
    if ns_display_anchor not in source:
        raise RuntimeError("Could not locate integrated number-switch display point")
    source = source.replace(ns_display_anchor, ns_display, 1)

    ns_submit_anchor = '''            def submit_noun_answer():
                recognition_answer = None
                parsed_answer = None
                evaluation = None
'''
    ns_submit = '''            def submit_noun_answer():
                recognition_answer = None
                parsed_answer = None
                evaluation = None
                ...
                if exercise_type == "number_switch":
                    ...
                    st.session_state.correct_answer = (
                        raw_answer if raw_answer and supplied in accepted
                        else "__agreement_number_switch_incorrect__"
                    )
'''
    if ns_submit_anchor not in source:
        raise RuntimeError("Could not locate integrated number-switch submit point")
    source = source.replace(ns_submit_anchor, ns_submit, 1)

    ns_feedback_anchor = '''                if exercise_type == "inflect":
'''
    ns_feedback = '''                if exercise_type == "number_switch":
                ...
                elif exercise_type == "inflect":
'''
    if ns_feedback_anchor not in source:
        raise RuntimeError("Could not locate integrated number-switch feedback point")
    source = source.replace(ns_feedback_anchor, ns_feedback, 1)
...
def apply_number_switch_base(source):
    ...
    number_prefix = r'''            if exercise_type == "number_switch":
                ...
                for possible_number, possible_case in sorted(matching_analyses):
                    ...
                    if target_number not in _ar_allowed_numbers(noun, adjective):
                        continue
                    for noun_target in _ar_forms(build_noun([noun, possible_case, target_number])):
                        for adjective_target in _ar_forms(
